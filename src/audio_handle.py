    ...
    def run(self):
        with soundfile.SoundFile(self._filename) as sound_data:
            ...
            secondary_device = sounddevice.OutputStream(samplerate=sound_data.samplerate,
                                                        dtype="float32",
                                                        channels=sound_data.channels,
                                                        device=device_number(options.get_additional_device_num()))
            info(f"Now playing {self._filename} ({sound_data.frames/sound_data.samplerate : 0.2f} s) on device {options.get_additional_device_name()}")
